...
from anamika.tools.telephony import list_sms
...
class TelegramBot:
    ...
    def __init__(self, config: Config, agent: Optional[Agent] = None):
        ...
        self.allowed_users = config.allowed_telegram_users
    ...
    def _request(self, method: str, data: Optional[Dict[str, Any]] = None, timeout: int = 30) -> Dict[str, Any]:
        """Makes an HTTP POST/GET request to Telegram Bot API."""
    ...
    def send_message(self, chat_id: int, text: str, parse_mode: Optional[str] = "Markdown") -> Dict[str, Any]:
        """Sends a text message to a Telegram chat."""
        # Split message if exceeds Telegram limit of 4096 characters
        if len(text) > 4000:
            chunks = [text[i:i+4000] for i in range(0, len(text), 4000)]
            res = {}
            for chunk in chunks:
                res = self._request("sendMessage", {
                    "chat_id": chat_id,
                    "text": chunk,
    ...
    def handle_message(self, message: Dict[str, Any]) -> None:
        ...
        if self.allowed_users and user_id not in self.allowed_users:
            print(f"🔒 Blocked unauthorized access from user_id: {user_id}")
            self.send_message(chat_id, "⛔ *Access Denied.* Unauthorized user ID.")
            return

        print(f"📩 [Telegram] User {user_id}: {text}")
        ...
        if text.startswith("/start") or text.startswith("/help"):
            help_text = (
                ...
                "💡 *Natural Language:* Aap mujhe seedhe Hinglish me bol sakte hain (e.g. _'Battery check karo'_, _'Back camera se photo lo'_, _'Torch on kar do'_, _'Recent OTP SMS dikhao'_, _'WhatsApp kholo'_)."
            )
        ...
        if text.startswith("/battery"):
            bat = get_battery_status()
            self.send_message(chat_id, f"🔋 *Battery:* `{json.dumps(bat, indent=2)}`")
            return
    ...
    def run(self) -> None:
        ...
        while self.running:
            try:
                ...
                print(f"⚠️ Telegram Polling Warning: {e}")
                time.sleep(3)
